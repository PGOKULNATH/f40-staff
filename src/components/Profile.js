...
const Profile = () => {
  ...
  const onChangepwd = () => {
    ...
    axios.post(server + '/otprequestmentor',user)
    .then(()=>{
  ...
  const addAchievement = (event) => {
    const headers={"Content-Type": "application/json","X-Access-Token":localStorage.getItem('ftoken')};
    event.preventDefault();
    const achieve = {achievement};
    toggleAddAchievementModal(!isAddAchievementModalOpen);
    profile_loading = true;
    axios.post(server+'/addachievement?rollNo='+localStorage.getItem('fmente'), achieve, {headers})
    .then(() => {
      setTimeout(() => {
        getProfile()
      },1000)
  ...
  const deleteAchievement = (event) => {
    ...
    profile_loading = true;
    axios.post(server+'/removeachievement?rollNo='+localStorage.getItem('fmente'), id, {headers})
    .then(() => {
      getProfile()
      setDeleteId(1)
    });
  ...
  const studentProfile = () => {
    ...
    return(
      <div className="container">
        <center>
          <h1>{localStorage.getItem('fmente')} Profile</h1>
        </center>
